...
import { Check, Upload, Image, Film, Music, Play, ChevronLeft, ChevronRight, Heart, X, Sparkles } from "lucide-react";
...
const TRANSITIONS = ["부드러운 페이드", "디졸브", "슬라이드", "전환 없음"];
...
function StepBody({ step, st }) {
            ...
            <div className="space-y-1.5">
              {st.uploads.map((u) => (
                <div key={u.id} className="flex items-center gap-2.5 px-3 py-2" style={{ background: SURFACE, border: "1px solid " + LINE, borderRadius: RADIUS }}>
                  {u.kind === "photo" ? <Image className="h-4 w-4 shrink-0" style={{ color: MUTE }} /> : <Film className="h-4 w-4 shrink-0" style={{ color: MUTE }} />}
                  ...
                  <button onClick={() => st.removeUpload(u.id)} className="p-0.5 outline-none" style={{ color: FAINT }} aria-label="삭제"><X className="h-4 w-4" /></button>
                </div>
              ))}
            </div>
          </>
  ...
  // 2 — AI 변환 (사진 3장 업로드 → 1장 타이틀 선택, 나머지 2장은 영상 변환)
  if (step === 2)
    return (
      <div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2.5">
          {TRANSITIONS.map((t, i) => {
            const on = st.trans === i;
            return (
              <button key={t} onClick={() => st.setTrans(i)} className="flex h-20 flex-col items-center justify-center gap-1.5 outline-none transition focus-visible:ring-1" style={{ background: on ? GOLD_SOFT : SURFACE, border: "1.5px solid " + (on ? GOLD : LINE), borderRadius: RADIUS }}>
                <Film className="h-5 w-5" style={{ color: on ? GOLD_D : MUTE }} />
                <span className="text-[12px] font-semibold" style={{ color: on ? GOLD_D : INK }}>{t}</span>
              </button>
            );
          })}
        </div>
    ...
    return (
      <div>
        ...
        <div className="mt-3 space-y-1 px-3 py-2.5 text-[11.5px]" style={{ background: "#f6f3ec", border: "1px solid " + LINE, borderRadius: RADIUS, color: MUTE }}>
          ...
          <div className="flex justify-between"><span>장면 전환</span><span style={{ color: INK }}>{TRANSITIONS[st.trans]}</span></div>
...
export default function UserMobile() {
  ...
  const [titleSel, setTitleSel] = useState(0);          // 업로드한 3장 중 타이틀로 쓸 사진 인덱스 (나머지 2장은 영상 변환)
  const removeUpload = (id) => setUploads((p) => p.filter((u) => u.id !== id));
  const addUpload = () => setUploads((p) => {
    const n = p.length + 1;
    const photo = p.filter((u) => u.kind === "photo").length;
    return [...p, { id: "u-" + Date.now(), name: `새 사진 ${photo + 1}.jpg`, kind: "photo", size: (1.5 + (n % 4) * 0.7).toFixed(1) + "MB" }];
  });
  const st = { agreed, setAgreed, uploads, removeUpload, addUpload, trans, setTrans, bgm, setBgm, letter, setLetter, titleSel, setTitleSel };

  const last = STEPS.length - 1;
  const previewStep = last - 1; // 미리보기 직전(편지) 버튼 라벨용
  const blocked = step === 0 && !agreed; // 동의 전 진행 차단
      ...
      <div className="w-full" style={{ maxWidth: 390 }}>
        ...
        <div className="overflow-hidden" style={{ background: SURFACE, border: "1px solid " + LINE, borderRadius: 18, boxShadow: "0 10px 30px rgba(42,38,34,.10)" }}>
          ...
          <div className="flex flex-col items-center px-5 py-5 text-center" style={{ background: "#faf7f1", borderBottom: "1px solid " + LINE }}>
            <Logo height={30} />
            <div className="mt-2.5 text-[14px] font-bold" style={{ color: INK }}>추모 영상 제작</div>
