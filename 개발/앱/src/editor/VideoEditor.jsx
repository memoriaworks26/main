...
export default function VideoEditor({ reservation, onClose }) {
  ...
  const { blocks, tplBgmId, partnerId, tplBgmVol } = useMemo(() => {
    ...
    return {
      ...
      tplBgmId: tpl.bgm || null,       // 템플릿 기본곡(=DB templates.bgm_id) — 이 영상에 지정이 없을 때 워커가 쓰는 폴백
      partnerId: pid,
      tplBgmVol: tpl.bgmVol ?? 70,      // 최종 렌더 BGM 음량(워커 tpl.bgm_volume) — 슬라이드 미리보기 볼륨을 이 값과 맞춤
    };
  ...
  // 실효 배경음악 — 워커와 동일 우선순위(이 영상 지정 submissions.bgm_id → 템플릿 기본 templates.bgm_id).
  //   슬라이드(사진) 구간이 있는데 실효 BGM이 없으면 그 구간은 무음으로 렌더된다 → 표시·경고에 사용.
  const effBgmId = media?.bgmId || tplBgmId || null;
  const effBgm = effBgmId ? store.bgm.find((b) => b.id === effBgmId) || null : null;
  // 무음 판정은 id 존재 기준 — BGM 라이브러리(store.bgm)가 아직 로드 전이어도 오탐(잘못된 무음 경고) 방지.
  const bgmName = effBgm ? effBgm.name : (effBgmId ? "배경음악 지정됨" : "없음 · 무음");
  // 추억 슬라이드 미리보기용 BGM 재생 URL — 최종 렌더처럼 슬라이드 재생에 소리를 깔아 편집기에서 미리 들을 수 있게.
  //   dev/목업=번들 음원(effBgm.src), 라이브=memoria-content 서명URL. 곡이 바뀔 때만 재발급(클립 URL과 동일 패턴).
  const [bgmUrl, setBgmUrl] = useState(null);
  useEffect(() => {
    let alive = true;
    if (!effBgm) { setBgmUrl(null); return; }
    if (effBgm.src) { setBgmUrl(effBgm.src); return; }              // 번들 음원(개발/목업)
    if (!effBgm.storagePath) { setBgmUrl(null); return; }
    storage.signedUrl(storage.BUCKETS.content, effBgm.storagePath)
      .then((u) => { if (alive) setBgmUrl(u); })
      .catch(() => { if (alive) setBgmUrl(null); });
    return () => { alive = false; };
  }, [effBgm?.id, effBgm?.src, effBgm?.storagePath]); // eslint-disable-line react-hooks/exhaustive-deps
  const hasSlide = blocks.some((b) => b.type === "slide");
  const slideSilent = hasSlide && !effBgmId;   // 슬라이드가 있는데 깔릴 BGM이 없음 = 무음 렌더
  ...
  //   편집기에서 클립을 교체하면 edits[블록id].assetId 오버라이드가 editedBlocks에 반영되므로 그걸 기준으로 URL을 발급.
  const [clipUrls, setClipUrls] = useState({}); // blockId → { kind:"videos"|"image", url, label }
  const clipBlocks = useMemo(() => editedBlocks.filter((b) => b.type === "clip"), [editedBlocks]);
  // id:assetId 조합만 키로 — 이게 바뀔 때만 서명URL 재발급(자막·음량 등 다른 편집엔 무영향).
  const clipKey = useMemo(() => clipBlocks.map((b) => b.id + ":" + (b.assetId || "")).join("|"), [clipBlocks]);
  useEffect(() => {
    let alive = true;
    const clips = clipBlocks.filter((b) => b.assetId);
    ...
    return () => { alive = false; };
  }, [clipKey, store.content]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // 렌더가 도는 동안(블록 AI 생성 or 최종 합성)엔 발행·컨펌도 잠근다 — 미완성 결과가 발행되는 것 방지.
  const renderBusy = ["queued", "rendering", "compose_queued", "composing"].includes(subStatus);
  const setStage = (next) => {
  ...
  const requestConfirm = async () => {
    ...
    // 무음 가드 — 추억 슬라이드(사진)가 있는데 이 영상에 깔릴 배경음악이 없으면, 그 구간이 무음으로 렌더된다.
    //   (오른쪽 「이 영상 배경 음악」에서 곡을 고르거나 템플릿 기본곡을 지정하면 됨.)
    if (!seJob && slideSilent && !(await confirm({ title: "배경음악 없이 렌더할까요?", message: "이 영상엔 배경음악이 지정되지 않아 추억 슬라이드(사진) 구간이 무음으로 렌더됩니다.\n오른쪽 「이 영상 배경 음악」에서 곡을 고를 수 있어요.", danger: true, confirmLabel: "무음으로 진행" }))) return;
    ...
    <div className="flex flex-col" style={{ height: "calc(100vh - 44px)", background: BG }}>
      <div className="flex items-center justify-between px-4" style={{ background: MASTER, height: 52 }}>
        ...
        <div className="flex items-center gap-2">
          ...
          {stage === "rendering"
            ? <Btn size="sm" onClick={requestConfirm} disabled={renderBusy}><Send className="h-3.5 w-3.5" strokeWidth={2.4} /> {blockRendering ? "블록 생성 중…" : renderBusy ? "렌더 중…" : "최종 렌더 · 컨펌 요청"}</Btn>
            : stage === "confirm"
            ? <Btn size="sm" onClick={confirmPublish} disabled={renderBusy}><Check className="h-4 w-4" strokeWidth={2.4} /> {renderBusy ? "렌더 중…" : "확인 · 컨펌(발행)"}</Btn>
            : <Btn size="sm" onClick={publish} disabled={renderBusy}><Check className="h-4 w-4" strokeWidth={2.4} /> 확정·발행</Btn>}
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex flex-1 flex-col overflow-y-auto px-6 py-5">
          <Preview sel={sel} blocks={panelBlocks} gens={gens} name={name} sourceVideoUrl={sourceVideoUrl} blockMedia={blockMediaAll} subtitles={editedSubs} onSubEdit={setEdit} onSelSub={(id) => setSel({ scope: "subtitle", kind: "subtitle", id })} bgmUrl={bgmUrl} bgmVol={tplBgmVol} />
